...
\item \textbf{Tortoise SVN:} \url{teste}
\end{itemize}
...
During the analysis of both repositories, some not expected issues have been found, that compromise the correct versioning of those resources.

\begin{itemize}[noitemsep]
\item From local repository:
\begin{itemize}[noitemsep]
\item Archetypes with the same content, but with different MD5-Hash. Thos can be due to some error from previous versions of ADL Designer, where this resources have been modeled or by some hand-made change on a text editor and then uploaded again to the local CKM. 
...
Because of this, when the information was inserted in the form by the user and sent to the server, with more than one day part per day, the archetype logic did not accept that value and it automatically threw an error describing that the added value was not supported. The updated archetype, had defined for magnitude "<|>=1.0|>", which means bigger or equal than 1. This means that this outdated archetype needs to urgently be substituted be the new version on the local repository.

After some analysis of the users of CKM repositories, was defined that the usual user of the script to update the archetypes would be the person responsible for taking care of the governance of the local openEHR artifacts repository on in their respective project, role usually performed by a clinical modeler, health informatician or even a programmer (when access to the first two are not available). 

It starts with one of these users being the "CKM repository owner", opening and login on the "ADL Designer" web application, choosing the desired repository and entering on the artifacts page from that repository. A button "Check archetypes updates from OpenEHR CKM" on the top right of "ADL Designer" should be click-able. When the button is clicked, a script will run to check if there are new updates from the list of archetypes on the repository on "ADL Designer", fetching from GitLab REST API, and comparing them with the same archetypes in the GitHub repository of openEHR international CKM, using also the REST API service. 

If there are updates, a new window frame will appear in the browser and let the user know which archetypes have new versions. Clicking in one of the archetypes mentioned in the window frame, it will open a new window in the browser and the user can compare both archetypes side by side, with highlighted lines in the changed code, which allows the user to check what were the main changes in the new version quickly. Then, if the user wants to download this new archetype to his computer, a button to download it will be available under the comparison page. \\

\begin{figure}[H]
	\centering
    \includegraphics[width=1\textwidth]{img/use_case.PNG}
...
If a experienced openEHR clinical modeler is used to this type of errors, it can be corrected quickly. Usually, if detected in a earlier stage by constant checking for updates on openEHR CKM, can save several time and money on projects.
